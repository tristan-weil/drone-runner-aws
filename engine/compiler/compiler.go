...
func (c *Compiler) Compile(ctx context.Context) *engine.Spec {
	...
	spec := &engine.Spec{
		AWS: engine.AWS{
			Endpoint:        c.Pipeline.AWS.Endpoint,
			Region:          c.Pipeline.AWS.Region,
			AccessKeyID:     c.Pipeline.AWS.AccessKeyID.Value,
			SecretAccessKey: c.Pipeline.AWS.SecretAccessKey.Value,
		},

		Server: engine.Server{
			// basics
			Name: fmt.Sprintf("drone-temp-%s", random()),
			AMI:  c.Pipeline.Server.AMI,
			Type: c.Pipeline.Server.Type,

			// spot
			SpotPrice: c.Pipeline.Server.SpotPrice.Value,

			// capacity
			DiskSize: c.Pipeline.Server.DiskSize,
			DiskName: c.Pipeline.Server.DiskName,

			// credentials
			Username:         c.Pipeline.Server.Username,
			Password:         c.Pipeline.Server.Password.Value,
			SSHPrivateKey:    c.Pipeline.Server.SSHPrivateKey.Value, // in PEM format
			SSHKeyType:       c.Pipeline.Server.SSHKeyType,
			SSHPublicKeyName: c.Pipeline.Server.SSHPublicKeyName,

			// network
			VPC:                 c.Pipeline.Server.VPC,
			SecurityGroupsNames: c.Pipeline.Server.SecurityGroupsNames,
		},
	...
	switch {
	case spec.Server.Username == "" && spec.Platform.OS == "windows":
		spec.Server.Username = "Administrator"
	case spec.Server.Username == "":
		//spec.Server.Username = "root"
		//spec.Server.Username = "admin"
		spec.Server.Username = "ec2-user"
	}

	if spec.Server.DiskName != "xvda" && spec.Server.DiskName != "/dev/xvda" {
		spec.Server.DiskName = "/dev/xvda"
	}

	// maybe load some secrets
	if s, ok := c.findSecret(ctx, c.Pipeline.AWS.AccessKeyID.Secret); ok {
		spec.AWS.AccessKeyID = s
	}
	if s, ok := c.findSecret(ctx, c.Pipeline.AWS.SecretAccessKey.Secret); ok {
		spec.AWS.SecretAccessKey = s
	}
	if s, ok := c.findSecret(ctx, c.Pipeline.Server.Password.Secret); ok {
		spec.Server.Password = s
	}
	if s, ok := c.findSecret(ctx, c.Pipeline.Server.SSHPrivateKey.Secret); ok {
		spec.Server.SSHPrivateKey = s
	}
	if s, ok := c.findSecret(ctx, c.Pipeline.Server.SpotPrice.Secret); ok {
		spec.Server.SpotPrice = s
	}

	// gen a key if missing
